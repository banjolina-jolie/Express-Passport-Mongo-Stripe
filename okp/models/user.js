...
User.prototype.validate = function(_user, done){
  ...
  Seq()
    .seq(function(){
      if(![constants.userTypes.LISTENER, constants.userTypes.PRESENTER].any(_user.type)){
        let result  = {result : false,
                       message : "New users have to be either a listener or presenter."};
      ...
      if(!_user.email ||
         ...
         !_user.username ||
         !_user.password ||
         !_user.hasOwnProperty('type')){
        let result  = {result : false,
                       message : "New users need at least a name, email, username, password and a type"};
        return done(null, result);
      }
      this();
    })
...
User.Create = function(_user, ip, done){
  ...
  if(!_user.email || !_user.username)
    return done(null, {result : false,
                       message : "New users need at least an email or username"});
  ...
  Seq()
    ...
    .seq(function(_hash){
      ...
      user.hash = _hash;
      user.tos_acceptance = {date: Math.floor(Date.now() / 1000),
                             ip : ip};
      ...
      user.state = user.type === constants.userTypes.SUPER ? constants.userTypes.ACTIVE : constants.userTypes.NEEDS_EMAIL_VERIFICATION;
      self.collection_.insert(user, this);
    })
    .seq(function(){
      // look for any meetings with this user's email as the listener attribute and adjust those meetings
      Meeting.convertEmailtoFullListener(user, this);
    })
    .seq(function(){
      ...
      emailManager.sendValidationEmail(user, function(err){
        ...
        self.collection_.update({email : user.email},
                                {$set : {state : constants.userStates.AWAITS_EMAIL_VERIFICATION}}, function(err){
                                  that(err);
                                });
      });
    })
...
User.canLogIn = function(user, forAdmin){
  if(forAdmin && user.type !== constants.userTypes.SUPER)
    return {result : false, message : "User is not of type SUPER"};
...
// TODO (Not needed ?)
// Hardcoding titles for the sake of the UI for now
User.mungeTitle = function(_user){
  let munged = "";
  if(_user.type === constants.userTypes.LISTENER)
    munged = "Dr.";
  else
    munged = "Rep.";
  return munged + " " + _user.name + " " + _user.surname;
};
